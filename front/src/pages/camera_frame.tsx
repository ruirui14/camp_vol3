...
import { Download, RefreshCcw, Loader2, ArrowLeft } from "lucide-react";
...
interface BodySegmentationMask {
...
interface BodySegmentationSegmentation {
  mask: BodySegmentationMask;
}

interface BodySegmenterConfig {
...
interface BodySegmenterType {
  segmentPeople(
    input: tf.Tensor3D | ImageData | HTMLVideoElement | HTMLImageElement | HTMLCanvasElement,
    segmentationConfig?: BodySegmenterConfig
  ): Promise<BodySegmentationSegmentation[]>;
  dispose(): void;
}

declare global {
  interface Window {
    FaceMesh?: new (config: { locateFile: (file: string) => string }) => CustomFaceMesh;
    bodySegmentation?: {
      SupportedModels: {
...
interface CropMetadata {
  ...
  headOriginalWidth: number;
  headOriginalHeight: number;
  bodyOriginalX: number;
  bodyOriginalYStart: number;
  bodyOriginalWidth: number;
  bodyOriginalHeight: number;
  originalSnapshotWidth: number;
  originalSnapshotHeight: number;
  overlapUsed: number;
}
...
const HORIZONTAL_SHIFT_FACTOR = 0.03;
...
type View = "camera" | "previewModal" | "compositeResult";

export const CameraFrames: React.FC = () => {
  ...
  const faceMeshRef = useRef<CustomFaceMesh | null>(null);
  const segmenterRef = useRef<BodySegmenterType | null>(null);
  ...
  const checkAlignmentWithMediaPipe = useCallback((landmarks: NormalizedLandmark[]) => {
  ...
  const performCropping = useCallback(
    async (
      ...
      segmenterInstance: BodySegmenterType | null
    ): Promise<{ headUrl: string | null; bodyUrl: string | null; meta: CropMetadata | null }> => {
      if (
        !segmenterInstance ||
      ...
      let segmentationResult: BodySegmentationSegmentation[];
      try {
        const segmentationConfig: BodySegmenterConfig = {
          flipHorizontal: false,
          multiSegmentation: false,
          segmentBodyParts: false,
        };
        segmentationResult = await segmenterInstance.segmentPeople(
          sourceCanvas,
          segmentationConfig
        );
      } catch {
      ...
      if (!segmentationResult || segmentationResult.length === 0 || !segmentationResult[0].mask) {
        if (isComponentMountedRef.current)
      ...
      const horizontalShift = W * HORIZONTAL_SHIFT_FACTOR;
      const headCropX = Math.max(0, ovalMinX - headXPadding - horizontalShift);
      ...
      const currentMeta: CropMetadata = {
        headOriginalX: headCropX + horizontalShift,
  ...
  const handleCapture = useCallback(
    async (isAutoTriggered = false) => {
      ...
      if (latestLandmarksRef.current && segmenterRef.current) {
        ...
        if (!cropResult.bodyUrl && isComponentMountedRef.current) {
          // showAppMessage("胴体の切り抜きに失敗、または胴体部分がありませんでした。", "warning");
        }
      } else {
        if (isComponentMountedRef.current)
          showAppMessage(
    ...
    (results: FaceMeshResults) => {
      ...
      if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
        ...
        if (isReadyToShoot && countdownValue === null && videoWidth > 0 && videoHeight > 0) {
          const leftEyeLms = LEFT_EYE_INDICES_FOR_EAR.map((i) => landmarks[i]).filter(
            (lm) => lm
          ) as NormalizedLandmark[];
          const rightEyeLms = RIGHT_EYE_INDICES_FOR_EAR.map((i) => landmarks[i]).filter(
            (lm) => lm
          ) as NormalizedLandmark[];

          if (
            leftEyeLms.length === LEFT_EYE_INDICES_FOR_EAR.length &&
            rightEyeLms.length === RIGHT_EYE_INDICES_FOR_EAR.length
          ) {
  ...
  const initializeModels = useCallback(async () => {
      ...
      allModelScriptsPromise = (async () => {
        try {
          ...
          if (!window.tf || typeof window.tf?.loadGraphModel !== "function")
            throw new Error("TFJS loadGraphModel is not available after loading converter.");
    ...
    try {
      ...
      if (isComponentMountedRef.current && !segmenterRef.current) {
        ...
        const modelId = window.bodySegmentation.SupportedModels.MediaPipeSelfieSegmentation;
        const segmenter = await window.bodySegmentation.createSegmenter(modelId, {
          runtime: "tfjs",
          modelType: "general",
        });
  ...
  useEffect(() => {
    ...
    const setupAsync = async () => {
      ...
      try {
        ...
        try {
          ...
          console.warn("環境カメラの取得に失敗、前面カメラを使用します。", error);
          s = await navigator.mediaDevices.getUserMedia({
            video: {
              facingMode: "user",
              width: { ideal: 960 },
        ...
        if (videoRef.current) {
          ...
          onPlayingHandler = () => {
            if (!isComponentMountedRef.current || !videoRef.current) return;
            if (canvasOverlayRef.current && faceMeshRef.current) {
              handleResize();
              if (isComponentMountedRef.current)
          ...
          videoRef.current.onloadedmetadata = () => {
            ...
            videoRef.current.play().catch((playError) => {
              if (isComponentMountedRef.current)
                showAppMessage(
                  `ビデオ再生エラー: ${playError instanceof Error ? playError.message : String(playError)}`,
                  "error"
                );
            });
          };
    ...
    // TODO stale closureの可能性があるので修正しないといけないが、対処方法がわからない
  ...
  const simulateServerProcessing = useCallback(async () => {
    ...
    if (isComponentMountedRef.current) {
      ...
      showAppMessage("サーバーに胴体情報を送信中...", "info");
    }
    ...
    bodyImageForDimensions.onload = async () => {
      ...
      try {
        ...
        const serverEndpoint = "http://localhost:8787/api/transform/suit";

        if (
          serverEndpoint.includes("localhost:8787") ||
          serverEndpoint.includes("YOUR_SERVER_ENDPOINT")
        ) {
          showAppMessage("ローカル/フォールバック処理を実行します。", "warning", 3000);
        ...
        const processedImageDataFromServer = responseData.image as string;
        ...
        if (isComponentMountedRef.current) setIsProcessingDressUp(false);
      }
    };
      ...
      <div className="flex min-h-screen flex-col items-center justify-center bg-gray-800 p-4 font-sans text-gray-100">
        ...
        {compositeDataUrl ? (
          <img
            ...
            alt="着せ替え後画像"
            className="max-h-[70vh] max-w-lg rounded-lg border-2 border-purple-500 bg-white object-contain shadow-xl"
          />
        ) : (
              ...
              <div className="flex flex-col items-center">
                ...
                {capturedImageURL && (
                  <img
                    ...
                    alt="撮影プレビュー"
                    className="mx-auto max-h-[40vh] max-w-full rounded-lg border border-gray-200 bg-white object-contain"
                  />
                )}
              </div>
              <div className="flex flex-col items-center">
